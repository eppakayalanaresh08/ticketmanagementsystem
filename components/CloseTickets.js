...
const CloseTickets = () => {
  ...
  const filterTickets = (tickets, filter) => {
    const closedticketfilter = tickets.filter((eachcloseItem) => eachcloseItem.status === 'close')

    if (filter === 'local') {
      setFilteredTickets(closedticketfilter.filter(ticket => ticket.state === 'telangana'));
    } else if (filter === 'non-local') {
      setFilteredTickets(closedticketfilter.filter(ticket => ticket.state !== 'telangana'));
    } else {
      setFilteredTickets(closedticketfilter);
    }
  }
  ...
  // console.log(closedticketfilter,'close')
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      {loading ? (
        ...
        <>
          <Text style={{ fontSize: responsiveHeight(1.2), marginHorizontal: responsiveHeight(2), fontWeight: '500', marginTop: responsiveHeight(2), marginLeft: responsiveHeight(4), fontSize: responsiveHeight(1.6) }}>SELECT TICKETS</Text>
...
const styles = StyleSheet.create({
  ...
  headerText: {
    fontWeight: 'bold', width: responsiveHeight(11), fontSize: responsiveFontSize(1.1)
  },
  valueText: {
